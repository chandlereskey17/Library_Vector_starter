...
int checkout(int bookid, int patronid){
	...
	if (patrons[pat_num].number_books_checked_out == MAX_BOOKS_ALLOWED_OUT) {
		return TOO_MANY_OUT;
	}
	patrons[pat_num].number_books_checked_out++;
...
int enroll(std::string &name){
	...
	patron my_patron;
	my_patron.name = name;
	my_patron.number_books_checked_out = 0;
	my_patron.patron_id = nextPatronID;

	patrons.push_back(my_patron);
	savePatrons(patrons, PATRONFILE.c_str());
	return my_patron.patron_id;
}
